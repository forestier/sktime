...
if __name__ == "__main__":
    dataset = 'GunPoint'
    path_to_train_data = '/mnt/nfs/casimir-home/gits/sktime/forestier/sktime/sktime/datasets/data/' + \
                         dataset + '/' + dataset + '_TRAIN.ts'
    train_x, train_y = load_from_tsfile_to_dataframe(path_to_train_data)

    # load the test data
    path_to_test_data = '/mnt/nfs/casimir-home/gits/sktime/forestier/sktime/sktime/datasets/data/' + \
    ...
    batch_size = min(train_x.shape[0] // 10, 16)

    if len(train_x.iloc[0, 0].shape) == 1:
        input_shape = [train_x.iloc[0, 0].shape[0], 1]
    else:
        input_shape = train_x.iloc[0, 0].shape[0:]
