...
class PresplitFilesCV:
    """Helper class for iterating over predefined splits in orchestration. Yields predefined splits. If cv is specified
    yields predefined splits first and then applies cv.
    """
    def __init__(self, cv=None, check_input=True):
        # TODO input checks
        # TODO integrate into sklearn class structure of CV iterators, make signature of split method consistent
    ...
    def split(self, data):
        ...
        train = np.where(data.index == 'train')[0]
        test = np.where(data.index == 'test')[0]
        yield train, test

        # if cv is specified, yield predefined split first, and then apply cv
        if self.cv is not None:
            for train, test in self.cv.split(data):
                yield train, test
